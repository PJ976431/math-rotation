...
const state = {
  ...
  activity1Step: 1, activity3Page: 1, awaitingOPlacement: false
};

const a2State = {
  width: 0, height: 0, center: null, A: null, B: null,
  selectedCenterType: 'A', angle: 0, animating: false, rafId: null,
  draggingLine: false, dragStartAngle: 0, dragStartObjAngle: 0,
  fixedStars: [ { x: 0, y: 0 }, { x: 0, y: 0 }, { x: 0, y: 0 }, { x: 0, y: 0 }, { x: 0, y: 0 } ],
...
function drawSegment(context, A, B) {
  context.save(); context.strokeStyle = '#2d6cdf'; context.lineWidth = 4; context.lineCap = 'round';
...
function initActivity2FixedStars() {
  ...
  a2State.fixedStars = [
    { x: a2State.A.x, y: a2State.A.y - 4 * GRID }, 
    { x: a2State.A.x - 4 * GRID, y: a2State.A.y }, 
    { x: a2State.A.x, y: a2State.A.y + 4 * GRID }, 
    { x: a2State.B.x, y: a2State.B.y + 4 * GRID }, 
    { x: a2State.B.x, y: a2State.B.y - 4 * GRID }  
  ];
}
...
function drawActivity2() {
  ...
  if (a2ModeLineBtn && a2Canvas) { a2Canvas.style.cursor = 'grab'; }
}
...
function startA2Animation() {
  ...
  a2State.angle = 0; a2State.animating = true;
  const step = () => {
    a2State.angle += 2;
    const center = a2State.selectedCenterType === 'A' ? a2State.A : a2State.B;
    const A = rotatePoint(a2State.A, center, a2State.angle);
    ...
    if (a2State.angle >= 360) {
      a2State.angle = 360; drawActivity2(); a2State.animating = false;
      cancelAnimationFrame(a2State.rafId); a2State.rafId = null; return;
    }
...
function stopA2Animation() {
  ...
  if (a2State.rafId) { cancelAnimationFrame(a2State.rafId); a2State.rafId = null; }
}
...
function resetA2All() {
  stopA2Animation(); a2State.angle = 0; a2State.selectedCenterType = 'A';
...
function exportLineState() {
  ...
  return {
    ...
    baseA: { x: state.A.x, y: state.A.y }, baseB: { x: state.B.x, y: state.B.y },
...
async function submitActivity3ToServer(page) {
  ...
  } catch (err) { console.error(err); setMsg('网络错误，提交失败'); }
}
...
document.querySelectorAll('input[name="centerChoice"]').forEach(radio => {
  radio.addEventListener('change', () => {
    ...
    if (next === 'O' && !state.hasO) {
      setMsg('请先添加 O 点');
      const aRadio = document.querySelector('input[name="centerChoice"][value="A"]');
      if (aRadio) aRadio.checked = true; return;
    }
...
addOBtn?.addEventListener('click', () => {
  state.awaitingOPlacement = true; state.mode = 'line';
  setMsg('请点击网格点放置 O 点'); draw();
});
...
pageNavBtn?.addEventListener('click', () => {
  ...
  if (state.activity1Step === 1) {
    ...
    // 【修改2】JS动态渲染文本同步替换为“任务要求”
    if (taskText) taskText.textContent = '任务要求：画出线段AB绕点A逆时针旋转90°后的线段并拍照上传。';
    ...
    // 【修改2】JS动态渲染文本同步替换为“任务要求”
    if (taskText) taskText.textContent = '任务要求：在方格纸上画出旋转后的线段AB并拍照上传';
...
submitBtn?.addEventListener('click', async () => {
  ...
  } catch (err) { msgEl.textContent = '活动一提交失败'; }
});
...
starCanvas?.addEventListener('pointerdown', (e) => {
  ...
  if (state.activity3Page === 2 && state.awaitingOPlacement) {
    ...
    if (oRadio) oRadio.checked = true; setMsg('O 点已放置'); draw(); return;
  }
  if (state.mode === 'star') {
  ...
  if (state.mode === 'line') {
    ...
    if (state.activity3Page === 2 && state.hasO) {
      ...
      if (dO < 35) {
        ...
        state.lineDragOffset = { x: p.x - state.O.x, y: p.y - state.O.y }; return;
      }
    }
    if (dA < 35 || dB < 35) {
...
starCanvas?.addEventListener('pointermove', (e) => {
  ...
  if (state.mode === 'star' && state.draggingStarIndex >= 0) {
    state.stars[state.draggingStarIndex] = snapPoint({ x: p.x + state.dragOffsetX, y: p.y + state.dragOffsetY });
    draw(); return;
  }
  if (state.mode === 'line' && state.draggingO && state.activity3Page === 2 && state.hasO) {
    ...
    state.selectedCenterType = 'O';
    const oRadio = document.querySelector('input[name="centerChoice"][value="O"]');
    if (oRadio) oRadio.checked = true; draw(); return;
  }
  if (state.mode === 'line' && state.draggingLine) {
    ...
    state.angle = state.dragStartObjAngle + deltaAngle; draw();
  }
});
...
if (a2Canvas) {
  ...
  a2Canvas.addEventListener('pointermove', (e) => {
    ...
    a2State.angle = a2State.dragStartObjAngle + deltaAngle; drawActivity2();
  });
...
function showPanel(activity) {
  ...
  if (activity === 3) { setTimeout(() => { resizeCanvas(); draw(); }, 50); }
  if (activity === 2) { setTimeout(() => { resizeA2Canvas(); drawActivity2(); }, 50); }
}
...
window.addEventListener('resize', () => { resizeCanvas(); resizeA2Canvas(); });
